...
class DeepQNetwork():
    ...
    def learn(self):
        ...
        # q_next 是目标网络在下一个状态上的 Q 值
        # q_eval 是评估网络在当前状态上的 Q 值
        # transition把状态s, [a, r], s_堆叠为一维数组
        ...
        # increase epsilon to reduce exploration
        self.cost_his.append(loss)
        self.epsilon = self.epsilon + self.epsilon_increment if self.epsilon < self.epsilon_max else self.epsilon_max
        self.learn_step_counter += 1

    def plot_cost(self):
        plt.plot(np.arange(len(self.cost_his)), [x.detach().numpy() for x in self.cost_his])
        plt.ylabel('Cost')
